...
public class PastTutorial {
    ...
    MyPastContent value;
    public PastTutorial(int bindport, InetSocketAddress bootaddress, int bootport, int numNodes, boolean isMemory) throws Exception {

        // Generate the NodeIds Randomly
        env = new Environment();
        env.getParameters().setString("nat_search_policy","never");

        System.out.println(bindport + " " + bootaddress.toString() + " " + String.valueOf(bootport) + " " + String.valueOf(numNodes)
        + " " + String.valueOf(isMemory));
    ...
    public PastContent getMethod(String lookupKey) throws InterruptedException {
        ...
        System.out.println("This first handle is " + p.getLocalNodeHandle());
        PastryIdFactory localFactory = new rice.pastry.commonapi.PastryIdFactory(env);
        ...
        while (async.getPastContent() == null && current+5000>=env.getTimeSource().currentTimeMillis()) {
            System.out.println(async.toString());
            continue;
        }
        System.out.println(async.getPastContent() + " ASYNC CONTENT");
        System.out.println(localFactory.buildId(lookupKey).toString() + p.getLocalNodeHandle() + p.storage.exists(localFactory.buildId(lookupKey))+"/*+-");

        return(async.getPastContent());
    }

    public void putMethod(String key, final String value){
...
//        storedKey[ctr] = myContent.getId();
        ...
        Continuation async = new Continuation<Boolean[], Exception>() {
            ...
            public void receiveResult(Boolean[] results) {
                ...
                System.out.println("[Continuation]"+myContent + " successfully stored at " +
                        numSuccessfulStores + " locations.");
            }
